...
def replace_with_antowords():
    train_data = R.read_csv('saimese_seperate/input_data/train65k.csv')
    train_words = []
    words = []
    for i in tqdm(train_data):
        train_words.append(i[0])
        train_words.append(i[1])

    for i in tqdm(list(tokenizer.encoder.keys())):
        if i[1:] in train_words:
            words.append(i)
...
def get_word_to_id_map(if_replace, tokenizer: PreTrainedTokenizer, word_counts=None, max_words: int = -1):
    """
...
def petal():
    ...
    # required parameters
    parser.add_argument("--output_dir", default='AVS_output/', type=str,
                        help="The output directory. The verbalizers are written to a file 'verbalizer.json' in this "
                             "directory.")
    parser.add_argument("--data_dir", default='input_data_ovr/', type=str,
                        help="The input data dir. Should contain the data files for the task.")
    parser.add_argument("--model_type", default='roberta', type=str,
                        help="The model type")
    parser.add_argument("--model_name_or_path", default='C:/Users/jiany/Desktop/pet-master/roberta-large', type=str,
                        help="Path to pre-trained model or shortcut name")
    parser.add_argument("--task_name", default='yelp-full-ovr', type=str,
                        help="The name of the task to train selected in the list: " + ", ".join(PROCESSORS.keys()))
    parser.add_argument("--if_replace", default=True, type=str,
                        help="Whether to replace train data with antonym pairs")

    # verbalizer search hyperparameters
    parser.add_argument("--normalize", action='store_true',
                        help="Whether to normalize the loss as proposed in the paper. It is recommended to set this "
                             "to 'true'.")
    ...
    parser.add_argument("--score_fct", default='llr', choices=['llr', 'ce', 'random'],
                        help="The function used to score verbalizers. Choices are: the log-likelihood ratio loss "
                             "proposed in the paper "
                             "('llr'), cross-entropy loss ('ce') and 'random', which assigns random tokens to each "
                             "label.")
    ...
    parser.add_argument("--words_file", default=None, type=str,
                        help="Path to a file containing (unlabeled) texts from the task's domain. This text is used "
                             "to compute "
                             "verbalization candidates by selecting the most frequent words.")
    parser.add_argument("--max_words", default=10000, type=int,
                        help="Only the 10,000 tokens that occur most frequently in the task’s unlabeled data (see "
                             "--words_file) are "
    ...
    word2idx = get_word_to_id_map(args.if_replace,tokenizer, word_counts=word_counts, max_words=args.max_words)
    ...
    for pattern_id in args.pattern_ids:
        ...
        config = WrapperConfig(model_type=args.model_type, model_name_or_path=args.model_name_or_path,
                               wrapper_type='mlm',
        ...
        # print(np.array(pattern_logits).shape)
        # print(pattern_logits)
